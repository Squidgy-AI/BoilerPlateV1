def create_sub_acc(
    ...
    """
    Create a new sub-account using the GHL API.
    ...
    if response.status_code == 200:
        ...
        raise requests.exceptions.RequestException(
            f"Failed to create sub-account. Status code: {response.status_code}, Response: {response.json()}"
        )
